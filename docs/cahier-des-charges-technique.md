...
**Statut : EN LIVE** — produits, prix, Payment Links et webhook recréés en mode réel (voir [`scripts/README.md`](../scripts/README.md#passage-en-mode-live-2026-09-05)), scénario D actif. Un vrai paiement fait dès maintenant créer un vrai compte client. Le plan Make gratuit limitait à 2 scénarios actifs simultanément (bloquait D pendant que A et B tournaient) — passé au plan payant "Core" pour lever la limite, les 3 scénarios tournent maintenant en même temps. Facturation **avec TVA**, prix **TTC fixes** (9€/29€/79€, quelle que soit la TVA — décision explicite plutôt que HT + TVA en plus) via `tax_behavior: inclusive` sur chaque Price. Le tarif "fondateur" n'est **pas** implémenté via un Coupon Stripe : plus simple et plus standard, chaque prix créé aujourd'hui (9/29/79€) restera figé pour les abonnements qui l'utilisent même si le prix public change plus tard pour les nouveaux inscrits (comportement natif d'un Price Stripe) — il suffira de créer de nouveaux Price à un tarif plus élevé pour les 101e inscrits et de ne plus proposer les Payment Links actuels, sans toucher aux abonnés déjà en cours.
